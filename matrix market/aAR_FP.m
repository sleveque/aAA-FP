function [xnew, iter, tol, anorm_story, rnorm_story] = aAR_FP(x0, it_fp, it_aa, m, rtol, maxit, fpiter)
...
%         fprintf("iter, residual: %i, %e\n",k,norm_k / norm0)

        if norm_k / norm0 < rtol% norm_k < rtol%
            break
        end
...
%             fprintf('iter: %i, Anderson Acceleration\n',k)
            % empplying Matlab's backslash
            coeff = Rk \ rnew;

%             % employing normal equation
%             rhs = Rk' * rnew;
%             Matrix = Rk' * Rk;
%             coeff = Matrix \ rhs;
